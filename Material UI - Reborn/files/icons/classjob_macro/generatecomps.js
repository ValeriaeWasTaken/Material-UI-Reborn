...
const baseCompSimple = fs.readFileSync("./base_comp_simple.json", "utf8")

classJobs.forEach((value, key) => {
    generateCompFull(value.name, value.type);
    generateCompSimple(value.name);
})

function generateCompFull(name, type){
...
function generateCompSimple(name){
    var currentText = baseCompSimple;
    currentText = currentText.replaceAll(/%class%/g, name);

    fs.writeFile("./" + name + "_simple.comp", currentText, (error) => {});
}